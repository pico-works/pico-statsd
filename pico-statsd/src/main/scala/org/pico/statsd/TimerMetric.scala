...
sealed trait TimerMetric[A] {
  ...
  def send(client: StatsDClient, aspect: String, sampleRate: Option[Double], value: A, duration: FiniteDuration, extraTags: List[String]): Unit
}
...
object TimerMetric {
  ...
  def integral[A](toTags: A => List[String]): TimerMetric[A] = new TimerMetric[A] {
    ...
    def send(client: StatsDClient, aspect: String, sampleRate: Option[Double], value: A, duration: FiniteDuration, t: List[String]): Unit =
